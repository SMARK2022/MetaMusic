...
if args.usr_img_path != "None":
    ...
    User_img = cv2.resize(User_img, np.array(args.size))
...
os.system("mkdir " + args.workplace)
if not args.workplace:
    input(">> Cleanup the output folder?(Press ENTER to continue)")
for file in os.listdir(args.workplace):
    os.remove(args.workplace + "/" + file)

# Do it
model = load_vqgan_model(args.vqgan_config, args.vqgan_checkpoint).to(device)
jit = True if "1.7.1" in torch.__version__ else False
perceptor = (
    clip.load(args.clip_model, jit=jit)[0].eval().requires_grad_(False)
)
...
def checkin(i, losses):
    ...
    TF.to_pil_image(out[0].cpu()).save(
        args.workplace + "/" + args.picturename, pnginfo=info
    )
...
for i in range(len(beats)-1):
    ...
    length_cum += length
    iterations_num.append(int(length * iter_per_second + 0.5))
    print(
        ...
        length,
        length * iter_per_second,
        int(length * iter_per_second + 0.5),
    )
    audiotimestamp_lst.append(
        ...
        + str(length)
        + "  "
        + str(length * iter_per_second)
        + "  "
        + str(int(length * iter_per_second + 0.5))
    )
...
for a in range(len(audio_length)):
    ...
    # print(audio.shape)
    # input("Are you OK?")
    pMs.append(Prompt(torch.from_numpy(wav2clip.embed_audio(audio, wav2clip_model)), float(1.0), float("-inf")).to(device))
    # input("Go Now?")

    if a != 0:
        # Initial Image embedding
        prompt = args.workplace + "/" + args.picturename
        path, weight, stop = split_prompt(prompt)
        img = Image.open(path)
        pil_image = img.convert("RGB")
        img = resize_image(pil_image, (sideX, sideY))
        batch = make_cutouts(TF.to_tensor(img).unsqueeze(0).to(device))
        pMs.append(Prompt(perceptor.encode_image(normalize(batch)).to(device).float(), weight, stop).to(device))
        # print(batch)
    ...
    if args.output_video_fps > 9:
        ...
        output_videocut = re.compile("\.png$").sub(
            f"{a}.mp4", args.workplace + "/" + args.picturename
        )
        try:
            p = Popen(
                [
                    ...
                    "png",
                    "-r",
                    str(args.iterations_per_second),
                    "-i",
                    "-",
                    "-b:v",
                    "10M",
        ...
        output_videocut = re.compile("\.png$").sub(
            f"{a:03d}.mp4", args.workplace + "/" + args.picturename
        )
...
mp4_files = [file for file in os.listdir(args.workplace) if file.endswith(".mp4")]
mp4_files.sort()
with open("list-of-files.txt", "w") as f:
    for file in mp4_files:
        file = args.workplace + "/" + file
        f.write(f"file {file[:-4]}.ts\n")
        os.system(
            f"ffmpeg -i {file} -vcodec copy -acodec copy -vbsf h264_mp4toannexb {file[:-4]}.ts"
        )

tmp_ts_combine = args.workplace + "/" + "output.ts"
...
ffmpeg.concat(video, audio, v=1, a=1).output(args.video_output, strict="-2").run()
